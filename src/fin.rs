...
impl<N:Typed<T=Nat>> Typed for Fin<N>{
	fn reflect() -> String {format!("fin({})",N::reflect())}
	type T=Star;
}
/// Functional constructor for Fin
pub struct FinFn;
impl Typed for FinFn {
	fn reflect() -> String {format!("fin()")}
...
{
	fn reflect() -> String {format!("finnm({},{})",N::reflect(),F::reflect())}
	type T=Fin<F>;
}
...
pub struct FMax;
impl Typed for FMax {
	fn reflect() -> String {format!("FMax()")}
	type T=Pi<Nat,Comp<FinFn,SuccFn>>;
}
