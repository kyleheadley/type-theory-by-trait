...
impl<A:Typed,B:Typed> Typed for Product<A,B> {type T=Star;}

/// Pairs of values
pub struct Pair<A:Typed,B:Typed>(A,B);
impl<T1:Typed,T2:Typed,A:Typed<T=T1>,B:Typed<T=T2>> Typed for Pair<A,B> {type T=Product<T1,T2>;}

/// Projection of first value (parameterized by value type)
pub struct Fst<T1:Typed,T2:Typed>(T1,T2);
impl<T1:Typed,T2:Typed> Typed for Fst<T1,T2> {type T=Arrow<Product<T1,T2>,T1>;}
