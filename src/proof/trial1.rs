...
impl<U:Counter> AbstractType for Type<U> {type U = Next<U>;}
...
struct Eq<A,B>(A,B);
impl<P,A,B> Typed for Eq<A,B> where
...
// Reflexivity Axiom: a =A a
// (provide a, an element of A)
struct Refl<A>(A);
impl<P,A> Typed for Refl<A> where
	P : Counter,
	A: Typed<U=P>,
{
	type U = P;
...
impl<P,A,B> Typed for EqAxiom<A,B> where
	P : Counter,
	A : Typed<U=P>,
	B : Typed<U=P>,
{
...
// Symmetry Axiom: A = B -> B = A
struct Symm<A,B>(A,B);
impl<P,A,B> Typed for Symm<A,B> where
	P : Counter, // Proof Universe number
	A : Typed<U=Next<P>>,
	B : Typed<U=Next<P>>,
...
impl<A:Typed> Func<Refl<A>> for Symm<A::T,A::T> where
	Max : CounterFn2<A::U,A::U,C=A::U>,
{ type F = Refl<A>; }
impl<P,A,B> Func<EqAxiom<A,B>> for Symm<A::T,B::T> where
	P : Counter,
	A : Typed<U=P>,
	B : Typed<U=P>,
	Max : CounterFn2<P,P,C=P>,
