...
impl<A:AbsArrow,B:AbsArrow<T2=A::T1>> Typed for Comp<A,B> {
	fn reflect() -> String {format!("Comp")}
	type T=Arrow<B::T1,A::T2>;
}
...
impl<T1:Typed,T2:Typed,V:Typed<T=T2>> Typed for Const<T1,V> {
	fn reflect() -> String {format!("Const({})",V::reflect())}
	type T=Arrow<T1,T2>;
}
...
impl<T:Typed> Typed for Id<T> {
	fn reflect() -> String {format!("id()")}
	type T=Arrow<T,T>;
}
